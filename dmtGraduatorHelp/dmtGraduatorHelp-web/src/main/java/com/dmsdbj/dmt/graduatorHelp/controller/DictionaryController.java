    ...
    public ItooResult findById(@PathVariable String id) {
		try {
			...
			if (dictionaryEntity != null) {
				return ItooResult.build("0000", "查询该课程为空", DictionaryEntity);
			}else{
				return ItooResult.build("0000", "查询该课程成功", DictionaryEntity);
			}
        
        } catch (Exception e) {
            logger.error("",e);
			return ItooResult.build("1111", "查询该课程失败", DictionaryEntity);
        }
    }
}    
